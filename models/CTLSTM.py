...
class CTLSTM(nn.Module):
    ...
    def __init__(self, 
        ...
        # Optional static attribute encoder
        self.static_encoder = None
        self.static_embedding_dim = 0
        if self.static_input_size > 0:
            layer_sizes = static_embedding_layers or (
                [static_embedding_dim] if static_embedding_dim is not None else [static_input_size]
            )
            layers: List[nn.Module] = []
            in_features = static_input_size
        ...
        combined_input_size = input_size + self.static_embedding_dim
    ...
    def forward(self, x, static_inputs: Optional[torch.Tensor] = None):
        """
        Forward pass.
...
def create_model(input_size, config, static_input_size: int = 0):
    """
...
if __name__ == "__main__":
    # Test the model
    model = CTLSTM(input_size=6, hidden_size=64, num_layers=2, output_size=1)
